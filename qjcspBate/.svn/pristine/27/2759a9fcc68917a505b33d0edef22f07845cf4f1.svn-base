...
$(function(){
	 ...
	 $grid = $dg.datagrid({//父表
		width : 'auto',
		height :  $(this).height(),
		url : "statistics/paymentStatement/findOrdersPage",
		queryParams:{},
		rownumbers:true,
		fit:false,
		fitColumns:true,
		...
		columns : [ [
		    {field : 'companyName',title : '加盟商名称',align : 'center'},
...
/*             		{field : '1',title : '总毛利',align : 'center'},
     		...
     		{field : 'realMoney',title : '实付金额',align : 'center',formatter:function(value, row, index){
     			return row.realMoney?parseFloat(row.realMoney).toFixed(2) || row.realMoney:"";
     		}},
     		...
     		{field : 'payNumber',title : '支付流水号',align : 'center'},
...
//     			return json[row.payMode] || "优惠抵扣";
         	...
         	{field : 'orderChildStatus',title : '子订单状态',align : 'center', formatter:function(value, row, index){
     			...
     			return statusJson[row.orderChildStatus] || "未知状态";
     		}},
     		...
     		detailFormatter:function(index,row){  
                return '<div style="padding:2px"><table class="dddv"></table></div>'; 
    	            ...
    	            onExpandRow: function(index1,row){//子表，注意此时行索引为index1                                  
    	                ...
    	                dddv.datagrid({
    	                    url:"statistics/paymentStatement/findOrderDetail?orderChildNum="+row.childNum,
    	                    fitColumns:true,
    	                    singleSelect:true,
    	                    loadMsg:"正在加载,请稍后...",
    	                    width: 800,
...
function exportOrderList(){
	...
	var orderType =$('#orderType').combobox('getValue');
	var payMode =$('#payMode').combobox('getValue');
//	encodeURIComponent(encodeURIComponent(row.goodsName))
//	location.href="statistics/paymentStatement/orderListExport?companyName="+companyName+"&orderNum="+orderNum+
	location.href=urls['ctx']+"/statistics/paymentStatement/orderListExport?companyName="+encodeURIComponent(encodeURIComponent(companyName))+"&orderNum="+orderNum+
