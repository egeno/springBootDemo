...
public class RefundOrderService {
	...
	public Map<String, Object> refundOrderList(JSONObject res) {
		...
		if (StringUtils.isNoneBlank(customerId)) {
			...
			for (int i = refundOrderList.size()-1; i>=0; i--) {
				...
				if (refundReasonOrderChildStatus.equals(OrderStatus.TIMEOUT.value)) {
					...
					String orderChildNum = refundOrder.get("orderChildNum").toString();
					...
					for (int j = refundOrderList.size()-1; j >=0; j--) {
						...
						if (refundOrderList.get(j).get("orderChildNum").equals(orderChildNum)) {
	 ...
	 * 售后/退款订单详情
	 */
	public Map<String, Object> refundOrderDetail(JSONObject res) {
		Map<String, Object> returnContent = new HashMap<String, Object>();
	...
	public Integer getRefundOrderList(String customerId) {
		...
		for (int i = 0; i < returnData.size(); i++) {
			if(returnData.get(i).get("orderStatus").equals(OrderStatus.REFOUND.value)||returnData.get(i).get("orderStatus").equals(OrderStatus.OPERATION_NO_PASS.value)||returnData.get(i).get("orderStatus").equals(OrderStatus.FINANCE_PASS.value)){
				index++;
			}
		}
		return index;
