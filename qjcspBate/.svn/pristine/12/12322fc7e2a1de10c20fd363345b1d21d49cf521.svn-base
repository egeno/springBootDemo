...
import com.qjkj.qjcsp.service.wechatapi.OrderDetailWeChatService;
...
public class OrderDetailWeChatController {
    public static Logger logger = LoggerFactory.getLogger(OrderDetailWeChatController.class);

    @Autowired
    private OrderDetailWeChatService orderDetailWeChatService;
    
    @RequestMapping(value = "/getOrderDetailWX", method = RequestMethod.POST)
    public Map<String, Object> getOrderDetailWX(HttpServletRequest req) {
        ...
        try {
            ...
            if (!StringUtils.isEmpty(orderNum)) {
                Map<String,Object> returnMap=orderDetailWeChatService.getOrderDetailWX(orderNum);
                if (returnMap.size()>0)
                {
	...
	public Map<String, Object> getCustomerDetail(ServletRequest request) {
		...
		try {
			...
			if (customerId != null && !customerId.isEmpty()&&NumberUtils.isNumber(customerId)) {
				...
				Map<String, Object> returnData = orderDetailWeChatService.getCustomerDetail(customerId);
