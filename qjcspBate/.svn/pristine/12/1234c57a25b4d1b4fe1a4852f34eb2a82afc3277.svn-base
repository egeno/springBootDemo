...
import com.qjkj.qjcsp.service.wechatapi.BalancePayWeChatService;
...
@Controller
public class BalancePayWeChatController {
	
	private static Logger logger = LoggerFactory.getLogger(BalancePayWeChatController.class);
	@Autowired
	private BalancePayWeChatService balancePayWeChatService;
	
	@RequestMapping(value="/balancePayWX", method = RequestMethod.POST)
	@ResponseBody
	public Map<String, Object> balancePayWX(HttpServletRequest req){
		...
		try {
			...
			if(!json.containsKey("customerId")||!json.containsKey("realMoney")||
			   !json.containsKey("orderNum")||!json.containsKey("payPassword")){
				...
				map.put("requestMethod", "balancePayWX");
				return map;
			}
			...
			if (orderNum != null && (!orderNum.isEmpty()) && realMoneystr != null 
				...
				int count =balancePayWeChatService.checkHasUserWX(customerId,payPassword);
				if(count==0){
				...
				+"orderNum:"+orderNum+"-----"+"payPassword:"+payPassword);
					...
					if(b){
						...
						map=balancePayWeChatService.balancePayWX(orderNum, bd,customerId);
					}else{
						map.put("returnCode", "0");
