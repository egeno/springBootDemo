(function ($, window, document, undefined) {
            ...
            if (s.display !== 'inline') {
                ...
                if (s.showOnTap) {
                    ...
                    that.tap($elm, function () {
                    	show(beforeShow, $elm);
                    	//当日期滚动框显示的时候
                        $(".dw-li").attr("aria-selected",'false').removeClass("dw-sel");
                        var areaID=localStorage.getItem("areaID");
                        for(var i=0;i<$(".dw-li").length;i++){
                        	if($(".dw-li").eq(i).attr("data-val")==areaID){
                        		$(".dw-li").eq(i).attr("aria-selected",'true').addClass("dw-sel").attr("selected","selected");
                        		 var dwUlHeight=-(i*40)+"px";
                        	}
                        }
                        $('.dw-ul').css("transform","translate3d(0px,"+dwUlHeight+",0px)");
                    });
                }
            }
        };
    ...
    ms.classes.Frame.prototype._defaults = {
        // Localization
        lang: 'en',
        setText: "<a class='tickBox'><div class='tick'></div></a>",
