package com.qjkj.qjcsp.service.refund;
...
public class OrderRefundService {
	...
	public void orderRefundOperateReject(OrderRefund orderRefund) {
		...
		/*当总订单下面的子订单没有待退款时将总订单状态改为8(退款失败) start xlk
		Long orderId=orderMapper.selectorderIdByorderChildId(Long.valueOf(orderRefund.getOrderChildId()));
		//查询总订单下子订单状态为5的数量
		int count=orderMapper.selectCountByorderId(orderId);
		if(count==0){
			// 修改总订单状态为8
			TblOrder tblOrder=new TblOrder();
			tblOrder.setOrderId(orderId);
			tblOrder.setOrderStatus(OrderStatus.OPERATION_NO_PASS.value);
			orderMapper.updateByPrimaryKeySelective(tblOrder);
		}
		
		当总订单下面的子订单没有待退款时将总订单状态改为8(退款失败) end xlk*/
	}
		...
		if (map.get("refundReasonOrderChildStatus").toString().equals(OrderStatus.TIMEOUT.value)) {
			...
			if (map.containsKey("payMode")) {
				if (map.get("payMode").equals("3")) {
					...
					/*当总订单下面的子订单没有待退款时将总订单状态改为9(已退款) start xlk
					Long orderId=orderMapper.selectorderIdByorderChildId(Long.valueOf(orderChildId));
					//查询总订单下子订单状态为5的数量
					int count=orderMapper.selectCountByorderId(orderId);
					if(count==0){
						// 修改总订单状态为9
						TblOrder tblOrder=new TblOrder();
						tblOrder.setOrderId(orderId);
						tblOrder.setOrderStatus(OrderStatus.FINANCE_PASS.value);
						orderMapper.updateByPrimaryKeySelective(tblOrder);
					}
					
					当总订单下面的子订单没有待退款时将总订单状态改为9(已退款) end xlk*/
					/* 退款金额添加到用户账号中 */
	 ...
	 */
	public Map<String, Object> customerOrderRefundApply(JSONObject jsonObject) {
