...
import com.qjkj.qjcsp.service.wechatapi.PreissueCommitOrderWeChatService;
...
public class PreissueCommitOrderWeChatController {
	private static Logger logger = LoggerFactory.getLogger(OrderCommonController.class);
	
	@Autowired
	private PreissueCommitOrderWeChatService preissueCommitOrderWeChatService;
	
	@RequestMapping(value = "/preissueCommitOrderWX", method = RequestMethod.POST)
	public Map<String, Object> commitOrder(ServletRequest request) {
		Map<String, Object> returnJson = new HashMap<String, Object>();
		returnJson.put("requestMethod", "preissueCommitOrderWX");
		String postJson = RequestData.getRequestPostJson(request);
		try {
			JSONObject res = JSONObject.fromObject(postJson);
			Map<String, Object> returnData = preissueCommitOrderWeChatService.PreissueCommitOrderWX(res);
